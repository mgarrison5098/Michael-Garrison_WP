...
if ( function_exists( 'add_theme_support' ) ) {
    ...
    load_theme_textdomain( 'mgarrison', get_template_directory() . '/languages' );
}
...
function mgarrison_post_comments_feed_link() {
    return;
}

add_filter('post_comments_feed_link', 'mgarrison_post_comments_feed_link');
...
function mgarrison_enqueue_scripts() {
...
add_action( 'wp_enqueue_scripts', 'mgarrison_enqueue_scripts' );
...
function mgarrison_register_nav_menus() {
    register_nav_menus([
        'header' => 'Header',
        'footer' => 'Footer',
    ]);
...
add_action( 'after_setup_theme', 'mgarrison_register_nav_menus', 0 );
...
function mgarrison_nav_menu_args( $args ) {
...
add_filter('wp_nav_menu_args', 'mgarrison_nav_menu_args');
...
function mgarrison_button_shortcode( $atts, $content = null ) {
    $atts['class'] = isset($atts['class']) ? $atts['class'] : 'btn';
    return '<a class="' . $atts['class'] . '" href="' . $atts['link'] . '">' . $content . '</a>';
}

add_shortcode('button', 'mgarrison_button_shortcode');
...
function mgarrison_mce_buttons_2( $buttons ) {
...
add_filter('mce_buttons_2', 'mgarrison_mce_buttons_2');
...
function mgarrison_tiny_mce_before_init( $settings ) {
    $style_formats = [
        // [
        //     'title'    => '',
        //     'selector' => '',
...
add_filter('tiny_mce_before_init', 'mgarrison_tiny_mce_before_init');
